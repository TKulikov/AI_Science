...
from llama_index.core import (
    Settings, Document, PropertyGraphIndex, VectorStoreIndex,
...
def _check_credentials() -> None:
    if not FOLDER_ID or not API_KEY:
        missing = [n for n, v in (("FOLDER_ID", FOLDER_ID), ("API_KEY", API_KEY)) if not v]
...
class YandexLLM(CustomLLM):
    ...
    def metadata(self) -> LLMMetadata:
        return LLMMetadata(context_window=8192, num_output=2000, model_name=self.model_name)
...
def _extractor(llm: YandexLLM) -> DynamicLLMPathExtractor:
    return DynamicLLMPathExtractor(
        llm=llm, max_triplets_per_chunk=MAX_TRIPLETS, num_workers=NUM_WORKERS,
        allowed_entity_types=None, allowed_relation_types=None,
        allowed_relation_props=None, allowed_entity_props=None,
    )
...
def build_all_indexes_streaming(
    ...
    graph = PropertyGraphIndex(
        nodes=[], llm=llm, embed_kg_nodes=EMBED_KG_NODES,
        kg_extractors=[_extractor(llm)], show_progress=False,
    )
    ...
    try:
        graph.property_graph_store.save_networkx_graph(name=str(graph_dir / "graph.html"))
    except Exception:
        pass
...
def build_all_indexes(documents: List[Document]) -> tuple[PropertyGraphIndex, VectorStoreIndex]:
    llm = _init()
    nodes = _split(documents)

    graph = PropertyGraphIndex(
        nodes=nodes, llm=llm, embed_kg_nodes=EMBED_KG_NODES,
        kg_extractors=[_extractor(llm)], show_progress=True,
    )
